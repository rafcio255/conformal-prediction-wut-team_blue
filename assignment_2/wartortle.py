...
def calc_quantile_no_class(data_cal, alpha):
    ...
    qhat = np.quantile(np.abs(data_cal['PRED'] - data_cal['TARGET'])/uncertainty_function(data_cal, data_cal), q_level, method='higher')

    return qhat

def uncertainty_interval(data_val, data_cal, q):
    result = uncertainty_function(data_val, data_cal)
...
def uncertainty_function(data_val, data_cal):
    data0 = data_cal[data_cal['TARGET'] == 0]
    data1 = data_cal[data_cal['TARGET'] == 1]
    median0 = np.median(data0['PRED'])
    median1 = np.median(data1['PRED'])
    
    data_val['MEDIAN_DIFF0'] = np.where(data_val['0_IN_PRED_SET'], np.abs(data_val['PRED'] - median0), 0)
    data_val['MEDIAN_DIFF1'] = np.where(data_val['1_IN_PRED_SET'], np.abs(data_val['PRED'] - median1), 0)
    data_val['MEDIAN_DIFF'] = np.maximum(data_val['MEDIAN_DIFF0'], data_val['MEDIAN_DIFF1'])

    return data_val['MEDIAN_DIFF']
